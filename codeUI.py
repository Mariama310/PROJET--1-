...
def get_next_product_id():
    existing_ids = {product["product_id"] for product in products_data}
    new_id = 1
    while new_id in existing_ids:
        new_id += 1
    return new_id
...
def update_order_ids():
    ...
    global orders_data
    orders_data = [{"order_id": i + 1, **order} for i, order in enumerate(orders_data)]
...
def add_sale():
    ...
    date and prod_id and quant and price and client_id):
        ...
        and is_numeric_input(price) and is_numeric_input(client_id)):
            df = pd.read_csv('./test_class_sales.csv')
            size = df.shape[0] + 1
            new_id = size
...
def refresh_orders_table():
    ...
    for order in orders_data:
        orders_tree.insert("", tk.END, values=(order["order_id"], order["order_date"], order["client_id"], order["product_ids"]))
...
if __name__ == "__main__":
    ...
    add_button_sales = tk.Button(button_frame_sales, text="Ajouter Sale", command=add_sale)
    add_button_sales.pack(side=tk.LEFT, padx=5)

    delete_button_sales = tk.Button(button_frame_sales, text="Supprimer Sale", command=delete_sale)
    delete_button_sales.pack(side=tk.LEFT, padx=5)

    modify_button_sales = tk.Button(button_frame_sales, text="Modifier Sale", command=modify_sale)
    modify_button_sales.pack(side=tk.LEFT, padx=5)

    exit_button = tk.Button(button_frame_sales, text="Quitter", command=frame.quit)
    exit_button.pack(side=tk.LEFT, padx=5)  
